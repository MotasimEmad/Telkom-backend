...
final class ServiceTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('image', function ($model) {
                return '<img src="' . env('SERVICES_PATH').$model->image . '" alt="Service Image" class="w-10 h-10 rounded-full">';
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Service image', 'image'),

            Column::make('Service name', 'name')
                ->searchable()
                ->sortable(),

            Column::make('Service description', 'description')
                ->searchable()
                ->sortable(),
    ...
    {
        $service = Service::findOrFail($rowId);
        $this->redirect(route('services.edit', $service->id));
    }
