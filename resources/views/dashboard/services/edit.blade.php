...
@section('content')
    ...
    <div class="p-4 bg-white rounded-lg shadow-sm xl:p-8">
        <form id="my-form" class="mt-5" action="{{ route('services.update', $service->id) }}" method="post"
              enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="mb-4 text-sm">
                <div class="w-96 h-96 rounded-md border-2 border-dashed flex items-center justify-center overflow-hidden px-2">
                    <img src="{{ env('SERVICES_PATH'). $service->image }}" class="w-full">
                </div>

                <div class="font-bold my-4">Service Image</div>
            ...
            <div>
                <label for="name" class="block text-sm text-gray-700 mt-4 font-bold">Service name</label>
                <input value="{{ $service->name }}" type="text" name="name" required
